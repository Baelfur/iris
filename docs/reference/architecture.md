...
```json
{
  ...
  "database": "postgresql",
  "host": "iris-public-postgres-7d8b9-abcde",
...
1. **Build matrix** fans out across the 5 variants × 2 architectures (`linux/amd64`, `linux/arm64`) using QEMU + Buildx. GHA-cached pip layers keep multi-arch tractable. Pushes to `ghcr.io/baelfur/iris-public-{variant}:{X.Y.Z}` and, for stable releases (no `-` in the version), `:latest`.
